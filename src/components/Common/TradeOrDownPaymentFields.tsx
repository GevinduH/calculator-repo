...
export default function TradeOrDownPaymentField({ componentName, className }) {
	...
	useEffect(() => {
		className === "DownPaymentField"
			? setInputValue(downPaymentValue)
			: setInputValue(tradeFieldValue);
	}, []);
	...
	return (
		<div className={className}>
			...
			<input
				type="text"
				value={inputValue}
				onChange={handleTradeorDownPayment}
				className="inputFields"
			/>
			{numericTradeVal > currMSRP * 0.25 ? (
				<p className="errorShow">
					...
					{componentName} cannot be greater than 1/4 of MSRP value
				</p>
			) : null}
